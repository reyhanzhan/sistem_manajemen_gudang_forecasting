      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex gap-2">
            <button onClick={() => setFilter('all')} className={`px-4 py-2 text-sm rounded-lg font-medium ${filter === 'all' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              All
            </button>
            <button onClick={() => setFilter('unread')} className={`px-4 py-2 text-sm rounded-lg font-medium flex items-center gap-1.5 ${filter === 'unread' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              Unread
            ...
            notifications.map((notif: any) => (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  {!notif.isRead && (
                    <button onClick={() => markRead(notif.id)} className="ml-4 p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded" title="Mark as read">
                      <Check size={16} />
                    </button>
                  )}
                </div>
