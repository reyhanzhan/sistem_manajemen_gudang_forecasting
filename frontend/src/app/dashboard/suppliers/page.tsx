      ...
      <div className="p-6 space-y-6">
        ...
        {showForm && (
          <div className="card border-2 border-primary-200">
            <h3 className="text-lg font-semibold mb-4">{editing ? 'Edit Supplier' : 'New Supplier'}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-2 lg:col-span-3"><label className="block text-sm font-medium text-gray-700 mb-1">Address</label>
              ...
              <tbody className="divide-y divide-gray-200">
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="text-center py-10 text-gray-400">
                    <Truck size={40} className="mx-auto mb-2 opacity-50" />No suppliers found
                  </td></tr>
                ) : suppliers.map((s: any) => (
                  <tr key={s.id} className="hover:bg-gray-50">
                    <td className="table-cell">
                      <div className="font-medium">{s.name}</div>
                      {s.contactPerson && <div className="text-xs text-gray-500">{s.contactPerson}</div>}
                    </td>
                    ...
                    <td className="table-cell text-sm">{s.city || '-'}</td>
                    <td className="table-cell text-sm text-center">{s._count?.productSuppliers || 0}</td>
                    <td className="table-cell">
                      <div className="flex gap-1">
                        <button onClick={() => edit(s)} className="p-1.5 text-gray-500 hover:text-primary-600 hover:bg-primary-50 rounded"><Pencil size={14} /></button>
                        <button onClick={() => handleDelete(s.id)} className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
